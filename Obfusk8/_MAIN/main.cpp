...
REG_CALLE(RegistryAPI* _RegistryAPI)
{
    char            readBuffer[256];
    HKEY            hKey;
    DWORD           bufferSize = sizeof(readBuffer), dwDisposition, type = 0;
...
{
    HCRYPTPROV hProv = 0, hHash = 0, hKey = 0;
...
ProcMan_CALLE(ProcessAPI* _ProcessManipulationAPIs)
{
    ...
    if (!_ProcessManipulationAPIs->pWriteProcessMemory(hProcess, remoteMem, OBFUSCATE_STRING("Obfusk8 C++17").c_str(), strlen(OBFUSCATE_STRING("Obfusk8 C++17").c_str()) + 1, &bytesWritten)) {
...
_main({
          ...
          cout << OBFUSCATE_STRING("\n-> [SYSCALL - INFO] K8 Syscall : Initializing...").c_str() << endl;

          DWORD pid = GetCurrentProcessId();
          cout << OBFUSCATE_STRING("-> [SYSCALL - INFO] Current PID: ").c_str() << pid << endl;

          CLIENT_ID_K8 cid;
          cid.UniqueProcess = (HANDLE)(uintptr_t)pid;
          cid.UniqueThread = 0;

          OBJECT_ATTRIBUTES_K8 oa;
          memset(&oa, 0, sizeof(oa));
          oa.Length = sizeof(oa);

          HANDLE hProcess = NULL;

          NTSTATUS status = K8_SYSCALL(
                  "ZwOpenProcess",
                  &hProcess,
                  PROCESS_ALL_ACCESS,
                  &oa,
          ...
          if (!status) {
              cout << OBFUSCATE_STRING("-> [SYSCALL - INFO] SUCCESS: ZwOpenProcess handle: ").c_str() << hProcess << endl;
              K8_SYSCALL("ZwClose", hProcess);
          }
         else
